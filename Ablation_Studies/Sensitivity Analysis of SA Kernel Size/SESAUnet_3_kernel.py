...
def adaptive_attention(input_tensor, name_prefix="attention"):
    ...
    attention = Conv2D(filters=1, kernel_size=(3, 3), padding='same', activation='sigmoid', name=f"{name_prefix}_conv")(concat)
    return Multiply(name=f"{name_prefix}_multiply")([input_tensor, attention])

def build_unet_sa_3x3(input_shape):
...
if __name__ == "__main__":
    model = build_unet_sa_3x3((512, 512, 3))
    model.summary()