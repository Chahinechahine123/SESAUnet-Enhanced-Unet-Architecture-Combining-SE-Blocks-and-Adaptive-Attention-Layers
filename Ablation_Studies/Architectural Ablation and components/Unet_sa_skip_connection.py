...
def decoder_block(input, skip_features, num_filters, block_name):
    ...
    # Apply SA to skip features BEFORE concatenation
    skip_features = spatial_attention(
        skip_features,
        name_prefix=f"{block_name}_skip_sa"
    )
...
def build_unet_sa_skip(input_shape=(512, 512, 3)):
    ...
    model = Model(inputs, outputs, name="UNet_SA_SkipConnections")

    return model

if __name__ == "__main__":
    model = build_unet_sa_skip()
    model.summary() 